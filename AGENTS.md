...
| `addVar` | `name`, `category`, `struct?`/`class?`/`sub?`, `container?`, `exposeOnSpawn?` | `AddMemberVariable` with a pin type built from the spec (category = bool/int/byte/real(+sub)/name/string/text/struct/object/class; `container` = Array/Set). `exposeOnSpawn:true` marks it instance-editable + `CPF_ExposeOnSpawn` so it surfaces as a pin on `CreateWidget`/`SpawnActor`/`ConstructObject` callers — then `connectPins` can drive it (commit the asset first so the caller's class reconstructs the new pin). | 4 |
